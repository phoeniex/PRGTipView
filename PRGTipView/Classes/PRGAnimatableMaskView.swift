...
public class PRGAnimatableMaskedView: UIView, CAAnimationDelegate {
  ...
  init(frame: CGRect, backgroundColor: UIColor, blurEffect: UIBlurEffect.Style?,
       maskFrame: CGRect, inset: UIEdgeInsets,
       isCircular: Bool, useLargestDimension: Bool,
       pulseMode: PRGTipViewFocusViewPulseMode, animationsDuration: Double) {
    ...
    if let visualBlurEffect = blurEffect {
      let blurEffectView = UIVisualEffectView(effect: UIBlurEffect(style: visualBlurEffect))
